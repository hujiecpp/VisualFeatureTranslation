...
def batch_get_feature(image_paths, config_path, output_dir, return_numpy_values=False):
  ...
  with tf.Graph().as_default():
    ...
    # sz = 
    # if sz[0] >= 1500 or sz[1] >= 1500:
    #   image_tf = tf.image.resize_images(image_tf, (800, 800))
    # if sz[0] < 224 or sz[1] < 224:
    #  image_tf = tf.image.resize_images(image_tf, (224, 224))

    print(image_tf)

    sz = tf.shape(image_tf)
    H = sz[0]
    W = sz[1]

    image_tf = tf.cond(tf.greater_equal(H, 1500), lambda: tf.image.resize_images(image_tf, (1000, 1000)), lambda: tf.cast(image_tf, tf.float32))
    image_tf = tf.cond(tf.greater_equal(W, 1500), lambda: tf.image.resize_images(image_tf, (1000, 1000)), lambda: tf.cast(image_tf, tf.float32))
    image_tf = tf.cond(tf.less(H, 224), lambda: tf.image.resize_images(image_tf, (224, 224)), lambda: tf.cast(image_tf, tf.float32))
    image_tf = tf.cond(tf.less(W, 224), lambda: tf.image.resize_images(image_tf, (224, 224)), lambda: tf.cast(image_tf, tf.float32))
...
def main(unused_argv):
  ...
  elapsed = (time.clock() - start)
  tf.logging.info('The time is: %f secs', elapsed)
