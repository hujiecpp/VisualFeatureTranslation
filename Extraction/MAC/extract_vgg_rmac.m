...
for i = 1 : size(white_list, 1)
  ...
  img = imread(img_name);
  vggrmac = vecpostproc(rmac_regionvec(img, net, L, 1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if dataset(1) == 'L'
        ...
        for i = 1 : size(base_list, 1)
          ...
          img = imread(img_name);
          vggrmac = vecpostproc(rmac_regionvec(img, net, L, 1));
          vggrmac = vecpostproc(apply_whiten(vggrmac, Xm, eigvec, eigval)); %
          save_name = ['../sdd/dtod/data/train/', dataset, '/base/', desc_name, '/', base_list(i).name, '.mat']

          %%%%%%%%
          % load(save_name);
          vggrmac = vecpostproc(sum(vggrmac, 2));
          % size(vggrmac)
          %%%%%%%%
