...
use_rvec = 1;  % use regional representation (R-MAC, R-GeM), otherwise use global (MAC, GeM)
use_gpu = [1];  % use GPUs (array of GPUIDs), if empty use CPU

network_file = './data/retrievalSfM120k-gem-vgg.mat';
...
for i = 1 : size(white_list, 1)
  ...
  img = imread(img_name);
  vggrgem = vecpostproc(descfun(img, net));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if dataset(1) == 'L'
        ...
        for i = 1 : size(base_list, 1)
          ...
          img = imread(img_name);
          vggrgem = vecpostproc(descfun(img, net));
          vggrgem = vecpostproc(apply_whiten(vggrgem, Xm, eigvec, eigval)); %
